class NotesController < ApplicationController
  ...
  def index
    ...
    if !current_user
      ...
      @notes = Note.all.limit(@@page_size).offset(@current_page * @@page_size ).all.sort_by(&:created_at).reverse
    end
  end
  ...
  def entry_params
    if current_user
      params.require(:note).permit(:subject, :body, :contact, :homesite, :public, :attachments)
    else 
      params.require(:note).permit(:subject, :body, :contact, :homesite, :attachments)
    end
  end
